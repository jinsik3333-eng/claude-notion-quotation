...
- 수락 기준과 완료 조건 정의
- **구현 → 테스트 → 검증 → 다음 단계** 순서 준수 (테스트 없이 다음 단계 진행 금지)
- **API 연동 및 비즈니스 로직 구현 완료 후 반드시 Playwright MCP 테스트 수행**
- **테스트 실패 시 즉시 수정 후 재테스트 (통과 전까지 진행 불가)**
- 각 단계별 Playwright MCP 스크린샷으로 결과 확인
...
#### API 연동 및 비즈니스 로직 구현 시 필수 테스트 순서

1. **개발 서버 실행 확인**
2. **Playwright MCP로 브라우저 접근**
3. **기능별 테스트 시나리오 수행**
4. **결과 스크린샷 및 검증**

#### 자주 사용하는 Playwright MCP 패턴

| 시나리오 | Playwright MCP 도구 |
|---------|-------------------|
| 페이지 접속 | `browser_navigate` |
| 화면 확인 | `browser_snapshot` or `browser_take_screenshot` |
| 버튼/링크 클릭 | `browser_click` |
| 폼 입력 | `browser_fill_form` or `browser_type` |
| API 응답 확인 | `browser_network_requests` |
| 콘솔 에러 확인 | `browser_console_messages` |
| 다이얼로그 처리 | `browser_handle_dialog` |

#### 테스트 체크리스트 템플릿 (Task 파일 내 `## 테스트 체크리스트` 섹션)

```
## 테스트 체크리스트

### 정상 플로우
- [ ] [시나리오 1]: browser_navigate → 페이지 렌더링 확인
- [ ] [시나리오 2]: API 응답 200 확인 (browser_network_requests)
- [ ] [시나리오 3]: UI 상태 변경 확인 (browser_snapshot)

### 오류 플로우
- [ ] 잘못된 입력 시 에러 메시지 표시 확인
- [ ] 존재하지 않는 리소스 접근 시 404 처리 확인
- [ ] 네트워크 에러 시 사용자 안내 메시지 확인

### 접근성
- [ ] 키보드 내비게이션 동작 확인
- [ ] 모바일 뷰 반응형 레이아웃 확인 (browser_resize)
```

#### ⚠️ 테스트 통과 없이 다음 단계 진행 금지

- API 연동 Task: 모든 정상/오류 플로우 테스트 통과 후 완료 처리
- 비즈니스 로직 Task: 엣지 케이스 포함 테스트 통과 후 완료 처리
- UI 구현 Task: 반응형 레이아웃 테스트 통과 후 완료 처리
...
- [ ] 구현 후 테스트 없이 Task 완료로 표시하는 경우가 없는가?
- [ ] 각 API 엔드포인트에 대해 정상/404/500 응답 테스트가 모두 포함되었는가?
- [ ] browser_network_requests로 실제 API 호출 및 응답 검증이 포함되었는가?
- [ ] 테스트 실패 시 수정 후 재테스트 프로세스가 명시되어 있는가?
